...
import { FaLock, FaEye, FaEyeSlash, FaCheck, FaArrowLeft, FaEnvelope, FaHeart, FaStar } from 'react-icons/fa';
...
const AdminResetPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log('🔄 Resetting admin password for email:', email);
      
      const response = await axios.post('http://localhost:8080/api/admin/reset-password-direct', {
        email: email,
  ...
  if (!email) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-red-400 via-orange-500 to-yellow-500 relative overflow-hidden">
        ...
        <div className="w-full max-w-md bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl border border-white/20 transform hover:scale-[1.01] transition-all duration-300 overflow-hidden">
          ...
          <div className="p-8 bg-gradient-to-r from-purple-500 to-pink-500 text-white text-center relative">
            ...
            <p className="text-pink-100 text-lg">
              {success ? "Password updated successfully!" : "Create your new admin password"}
            </p>
